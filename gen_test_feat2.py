...
def gen_user_feat():
    ...
    4.sex编码
    """
    dump_file='feat/test_user_feat.pkl'  #对t_user表数据处理保存
    if os.path.exists(dump_file):
        ...
        print(user.columns)
        user['day']=user['active_date'].map(lambda x:datetime.strptime('2016-12-1','%Y-%m-%d')-datetime.strptime(x,'%Y-%m-%d'))
...
def gen_loan_feat():
    loan_file='feat/test_loan_feat.pkl'
    if os.path.exists(loan_file):
        ...
        loan=loan[loan['month']!=8]#训练集排除11月份数据
        ...
        #每月贷款次数特征
        #1.上次贷款距离现在时间 算出贷款权重
        loan['distanceTimeWeight']=loan['loan_time'].map(lambda x:datetime.strptime('2016-12-1 00:00:00','%Y-%m-%d %H:%M:%S')-datetime.strptime(x,'%Y-%m-%d %H:%M:%S'))
        ...
        loan_time['loan_time']=loan_time['loan_time'].map(lambda x:datetime.strptime('2016-12-1 0:00:00','%Y-%m-%d %H:%M:%S')-datetime.strptime(x,'%Y-%m-%d %H:%M:%S'))
        ...
        loan_exceed['loan_exceed11'] = loan_exceed[11] - loan_exceed['limit']
        ...
        for id in list(loan.index):
            if loan.loc[id,'plannum']==1 and loan.loc[id,'month']<=10:
                loan.loc[id,'repay']=0
        ...
        loan = pd.concat([loan, loan_month], axis=1)
        loan=loan.groupby(['uid'],as_index=False).sum()
        loan['loan_sum_month91011'] = loan['month_11'] + loan['month_9'] + loan['month_10']  # 这几个特征get_dummies
        loan['loan_continue911'] = loan['month_11'] + loan['month_9']
        loan['loan_continue911'] = loan['loan_continue911'].map({0: 1, 1: 0, 2: 1})  # 是否连续贷款做映射
        loan['loan_continue1011'] = loan['month_11'] + loan['month_10']
        loan['loan_continue1011'] = loan['loan_continue1011'].map({0: 1, 1: 0, 2: 1})
        loan['loan_continue910'] = loan['month_9'] + loan['month_10']
        loan['loan_continue910'] = loan['loan_continue910'].map({0: 1, 1: 0, 2: 1})
        loan['loan_continue_month91011'] = loan['month_11'] + loan['month_9'] + loan['month_10']
        loan['loan_continue_month91011'] = loan['loan_continue_month91011'].map({0: 1, 1: 0, 2: 0, 3: 1})

        # 平均每月分期期数 平均每月贷款金额
        loan['per_month_plannum'] = loan['plannum'] / loan['loan_sum_month91011']
        loan['per_month_amount'] = loan['loan_amount'] / loan['loan_sum_month91011']
        ...
        loan=pd.merge(loan,loan_exceed[['uid','loan_exceed11','loan_exceed9','loan_exceed10']],how='left',on='uid')
...
def loan_timeWindow_feat():
    loan_timeWindow_file="feat/test_loanTimeWindow_feat.pkl"
    if os.path.exists(loan_timeWindow_file):
        ...
        loan_timeWindow=loan_timeWindow[loan_timeWindow['month']!=12]

        loan_timeWindow['interval_day']=loan_timeWindow['loan_time'].map(lambda x:datetime.strptime("2016-12-1 0:00:00","%Y-%m-%d %H:%M:%S")-datetime.strptime(x,"%Y-%m-%d %H:%M:%S"))
...
def gen_click_feat():
    click_file='feat/test_click.pkl'
    if os.path.exists(click_file):
        ...
        click_feat=click_feat[click_feat['click_month']!=12]
        ...
        #最后一次点击距离12月时常 距离11平均间隔 最后一次点击是否超过平均间隔
...
def gen_order_feat():
    order_file='feat/test_order.pkl'
    if os.path.exists(order_file):
        ...
        order_feat=order_feat[order_feat['month']!=12]
        ...
        # order_time['buy_time']=order_time['buy_time'].map(lambda x:datetime.strptime('2016-12-1','%Y-%m-%d')-datetime.strptime(x,'%Y-%m-%d'))
...
def gen_test_data():
    test_file='feat/testData.pkl'
    if os.path.exists(test_file):
        testData=pickle.load(open(test_file,'rb'))
    else:
        ...
        testData=pd.merge(user_data,loan_data,how='left',on='uid')
        testData=pd.merge(testData,loan_order,how='left',on='uid')
        testData=pd.merge(testData,loan_click,how='left',on='uid')
        testData=pd.merge(testData,loan_timeWindow,how='left',on='uid')
        #testData=pd.merge(testData,train_label,how='left',on='uid')

        pickle.dump(testData,open(test_file,'wb'))
        feat_id={id:feat for id,feat in enumerate(list(testData))}
        print(feat_id)
    return testData
...
if __name__=='__main__':
    ...
    # test_data=gen_test_data()#对测试集数据做了特征处理 没标签
